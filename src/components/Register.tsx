...
const checkUsernameAvailability  = async (username:string)=>{
  if (username.trim() === '') return; // Avoid sending a request for empty input
    setLoading(true);
try {
  const response  = await axios.get(`${import.meta.env.VITE_API_URL_USER_BASE_URL}/check-username/`,{params:{username}})
  ...
  */
  return (
    ...
    <div className="max-w-md mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
      ...
      />
      {loading ? <p>Checking availability...</p> : usernameAvailable === false && <p>Username is already taken.</p>}
      <input
        type="email"
        placeholder="Email"
        className="w-full p-3 mb-4 border rounded"
